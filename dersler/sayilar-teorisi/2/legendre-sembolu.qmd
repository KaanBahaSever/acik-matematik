...
pagetitle: "Legendre Sembolü ve Euler Kriteri"
...
```{=html}
<div class="calculator" id="legendre-calculator">
  <h4>Legendre Sembolü Hesaplayıcı</h4>

  <div class="calc-row">
    <label>
      <span>Tam sayı (a)</span>
      <input type="number" data-role="a" value="3">
    </label>
    <label>
      <span>Tek asal (p)</span>
      <input type="number" data-role="p" value="13" min="3">
    </label>
  </div>
  ...
  <p class="calc-note" data-role="explanation"></p>
</div>

<script>
(function () {
  const box = document.getElementById("legendre-calculator");
  const get = (role) => box.querySelector('[data-role="' + role + '"]');
  const aInput = get("a"), pInput = get("p");
  const output = get("output"), explanation = get("explanation");
  ...
  // Euler's criterion: a^((p-1)/2) mod p, by repeated squaring
  const modPow = (base, exponent, modulus) => {
    let result = 1n, t = ((BigInt(base) % BigInt(modulus)) + BigInt(modulus)) % BigInt(modulus);
    let e = BigInt(exponent), m = BigInt(modulus);
    while (e > 0n) {
      if (e & 1n) result = (result * t) % m;
      t = (t * t) % m;
      e >>= 1n;
    }
  ...
  function compute() {
    const a = Number(aInput.value), p = Number(pInput.value);

    if (!Number.isInteger(a) || !Number.isInteger(p) || p < 3 || !isPrime(p) || p === 2) {
      output.textContent = "—";
      explanation.innerHTML = '<span class="calc-error">p, 2\'den büyük bir asal sayı olmalıdır.</span>';
      return;
    }

    const residue = ((a % p) + p) % p;
    if (residue === 0) {
      output.textContent = "( " + a + " / " + p + " ) = 0";
      explanation.textContent = "p sayısı a'yı tam böldüğü için sembol 0 değerini alır.";
      return;
    }

    const exponent = modPow(residue, (p - 1) / 2, p);
    const value = exponent === 1 ? 1 : -1;

    output.textContent = "( " + a + " / " + p + " ) = " + value;
    explanation.textContent =
      "Euler kriteri: " + residue + "^" + ((p - 1) / 2) + " ≡ " + exponent + " (mod " + p + ")  ⇒  " +
      (value === 1 ? "a bir kuadratik rezidüdür (KR)." : "a bir kuadratik non-rezidüdür (KNR).");
  }

  [aInput, pInput].forEach((el) => {
