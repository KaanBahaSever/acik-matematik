...
::: {.callout-warning title="Kaynak nottaki yazım"}
(c) şıkkında pay $e^{-6}(-6)^4$ olarak yazılmıştır; taban **$-6$ değil $6$** olmalıdır. Üs çift olduğu için sayısal sonuç değişmemiş, ama Poisson formülünde taban her zaman $\lambda t > 0$'dır.
:::
::::
...
::: {.callout-warning title="Kaynak nottaki iki hata"}
**(a)** şıkkının sonucu $0{,}295$ olarak yazılmıştır; doğrusu $\dfrac{e^{-4} 4^3}{3!} = 0{,}1954$'tür.

**(f)** şıkkında yalnızca $\dfrac{e^{-36}\,36^{30}}{30!}$ tek terimi yazılmıştır. Bu ifade $\Pr\{Y_6 = 30\} = 0{,}0427$ değerini verir; oysa soru $\Pr\{Y_6 > 30\}$ istemektedir. "Fazla olma" olasılığı bir **kuyruk** olasılığıdır ve tümleyenden hesaplanır.
:::
::::
...
::: {.callout-warning title="(b) şıkkı için kaynak nottaki cevap"}
Kaynak notta bu şıkkın cevabı, "kamyonlar toplamın $\%5$'i ise $10$ kamyon $200$ araca karşılık gelir" akıl yürütmesiyle $200$ olarak verilmiştir.

Bu, oransal bir **tahmindir** ve Poisson varsayımıyla uyuşmaz: ayrıştırma teoremi gereği kamyon ve diğer araç süreçleri bağımsızdır, dolayısıyla $Y_{K,1} = 10$ bilgisi $Y_{D,1}$'in dağılımını hiç değiştirmez. Doğru cevap $10 + 57 = 67$'dir.

Sezgiye ters gelen bu sonuç aslında makuldür: o saatte beklenenden az kamyon geçmiş olması ($3$ yerine $10$ — burada tersine, fazla), diğer araçların sayısı hakkında bir şey söylemez.
:::
::::
...
::: {.callout-warning title="Eşitsizliğin yönü"}
Kaynak notta son satır $\dfrac{\log 9901}{\log 2} + 1 \geq n$ biçiminde, yani **ters yönde** yazılmıştır. Aranan "en az kaç kez" olduğu için eşitsizlik $n \geq \dfrac{\log 9899{,}5}{\log 2} + 1$ olmalıdır; aksi hâlde çözüm kümesi $n \leq 14{,}27$ çıkar ve soru anlamsızlaşır.
:::
::::
...
::: {.callout-warning title="(c) şıkkı için kaynak nottaki sonuç"}
Kaynak notta bu şıkkın sonucu $0{,}85$ olarak verilmiştir. Ancak (b) şıkkında bulunan $0{,}01614$ değeri **zaten $8$ saatlik** beklenen değerdir; bu yüzden tekrar $8$ ile çarpılmamalıdır. Doğrusu $e^{-0{,}01614} = 0{,}9840$'tır.

Sağduyu kontrolü de bunu destekler: $8$ saatte ortalama $0{,}016$ bozucu arıza bekleniyorsa, hiç arıza olmaması olasılığının $1$'e çok yakın çıkması gerekir.
:::
::::
