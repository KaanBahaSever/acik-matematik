...
::: {.callout-note title="Adlandırma"}
Kaynak notta teoremin adı "Chapinan–Kolmogrov" olarak yazılmıştır; doğrusu **Chapman–Kolmogorov**'dur (İngiliz matematikçi Sydney Chapman ve Rus matematikçi Andrey Kolmogorov).
:::
...
::: {.callout-warning title="Soru metni ile çözüm arasındaki tutarsızlık"}
Kaynak notta ikinci koşul "B marka içeceği satan bir firmanın $1$ hafta sonra tekrar B marka içecek satma olasılığı **$0{,}1$**" olarak yazılmış, fakat çözümde ikinci satırı $(0{,}2;\ 0{,}8)$ olan matris kullanılmıştır; yani orada $p_{BB} = 0{,}8$'dir.

Metindeki değer harfiyen alınsaydı $p_{BB} = 0{,}1$, dolayısıyla $p_{BA} = 0{,}9$ olur ve matrisin iki satırı özdeşleşirdi:

$$P = \begin{pmatrix} 0{,}9 & 0{,}1 \\ 0{,}9 & 0{,}1 \end{pmatrix} \implies P^n = P \;\; (n \geq 1)$$

Bu durumda cevaplar $0{,}1$ ve $0{,}1$ çıkardı — yani soru üç haftayı sormanın anlamını yitirirdi. Nottaki $P^{(3)}$ değerleri de ($0{,}78$ ve $0{,}22$) $p_{BB} = 0{,}8$ ile uyuşmaktadır. Bu yüzden yukarıda ikinci koşul $0{,}8$ alınmıştır.
:::
::::
