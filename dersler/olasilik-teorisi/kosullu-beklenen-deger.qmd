...
**(b)** **Lineerlik:** $E\big(a\, g(X, Y) + b\, h(X, Y) \mid X = x\big) = a\, E\big(g(X, Y) \mid X = x\big) + b\, E\big(h(X, Y) \mid X = x\big)$.
...
**(a)** $\displaystyle E(c \mid X = x) = \int_{-\infty}^{\infty} c\, f_{Y \mid X = x}(y)\,dy = c \int_{-\infty}^{\infty} f_{Y \mid X = x}(y)\,dy = c.$
...
Toplam varyans: $X = x$ verilmişken $Y - x$'in yoğunluğu $e^{-u}$, $u > 0$'dır; ikinci momenti $\int_0^{\infty} u^2 e^{-u}\,du = 2$ ve ortalaması $1$ olduğundan $\operatorname{Var}(Y \mid X = x) = 2 - 1 = 1$. Buradan $E[\operatorname{Var}(Y \mid X)] = 1$. Öte yandan $\operatorname{Var}[E(Y \mid X)] = \operatorname{Var}(1 + X) = \operatorname{Var}(X) = \int_0^{\infty} x^2 e^{-x}\,dx - 1 = 2 - 1 = 1$. Öyleyse $\operatorname{Var}(Y) = 1 + 1 = 2$. Doğrulama: $E(Y^2) = \int_0^{\infty} y^3 e^{-y}\,dy = 3! = 6$ ve $\operatorname{Var}(Y) = 6 - 4 = 2$.
...
$E(Y \mid X = x) = E(X + Z \mid X = x) = x + E(Z \mid X = x) = x + E(Z) = x + \frac{7}{2}$; burada @prp-kosullu-beklenen-ozellikleri (b), (c) ve bağımsızlık için (d) kullanıldı.

$E(X \mid Y = y)$ için bakışım kullanalım: $(X, Z)$'nin ortak dağılımı bileşenlerin yer değiştirmesine göre bakışımlıdır, dolayısıyla $Y = y$ verilmişken $X$ ile $Z$'nin koşullu dağılımları aynıdır ve $E(X \mid Y = y) = E(Z \mid Y = y)$. Lineerlikle $E(X \mid Y = y) + E(Z \mid Y = y) = E(X + Z \mid Y = y) = E(Y \mid Y = y) = y$; öyleyse $E(X \mid Y = y) = \dfrac{y}{2}$, $y \in \{2, \dots, 12\}$. (Doğrudan sayma ile de görülebilir: $Y = y$ verilmişken $X$, $\max(1, y - 6)$ ile $\min(6, y - 1)$ arasındaki tam sayılar üzerinde düzgün dağılır ve bu aralığın orta noktası $\frac{y}{2}$'dir.)
