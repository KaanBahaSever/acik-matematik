...
Bu, Cauchy'nin on dokuzuncu yüzyılın ilk yarısında elde ettiği sonuçtur.
...
İspat iki adımlıdır: önce bölgeyi, üzerinde fark bölümünün türevden çok az saptığı küçük karelerle örten bir lemma kurulur; sonra integral bu karelerin sınırları üzerinde dağıtılır.
...
::: {.ispat}
$\varepsilon > 0$ verilsin ve $R$, lemmadaki gibi örtülsün. $j$-inci alt bölgede, $z_j$ lemmadaki sabit nokta olmak üzere
...
*Yanıtlar:* a) tekil nokta $z = 3$ çemberin dışındadır; b) integrand tamdır; c) paydanın kökleri $-1 \pm i$ olup modülleri $\sqrt{2} > 1$'dir; d) dal kesimi, reel eksenin $\operatorname{Re} z \le -2$ parçasından oluşan ışındır ve çemberle kesişmez.
:::
