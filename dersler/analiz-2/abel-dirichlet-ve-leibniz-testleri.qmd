...
olacak biçimde bir $M > 0$ bulunsun. $(a_n)$ dizisi monoton (bkz. [Analiz 1](../analiz-1/diziler.html#def-monoton-dizi)) ve
...
olur; ortadaki toplam teleskopiktir (@def-teleskopik-seri). Üçgen eşitsizliği (bkz. [Analiz 1](../analiz-1/mutlak-deger.html#thm-ucgen-esitsizligi)) ve Adım 1 gereği
...
*Sonuç.* İç içe aralıklar teoremi (bkz. [Analiz 1](../analiz-1/supremum-infimum-ve-tamlik.html#thm-ic-ice-araliklar)) gereği bütün $I_n$'lerin arakesiti tam olarak bir $s$ noktasından oluşur. Her $n$ için $s_{2n}, s_{2n+1} \in I_n$ ve $s \in I_n$ olduğundan

$$|s_{2n} - s| \le |I_n| = a_{2n+1}, \qquad |s_{2n+1} - s| \le |I_n| = a_{2n+1}$$

olur. $a_{2n+1} \to 0$ olduğundan sıkıştırma teoremi (bkz. [Analiz 1](../analiz-1/limit-islemleri-ve-sikistirma.html#thm-sikistirma-dizi)) gereği
...
**Koşul 3: sıfıra yakınsama.** Arşimet özelliğinin bir sonucu olarak $\dfrac{1}{k} \to 0$'dır (bkz. [Analiz 1](../analiz-1/arsimet-ozelligi.html#cor-arsimet-bir-bolu-n)).
...
$\sum_{k=1}^{\infty} b_k$ serisi yakınsak, $(a_n)_{n \in \mathbb{N}}$ dizisi ise monoton (bkz. [Analiz 1](../analiz-1/diziler.html#def-monoton-dizi)) ve sınırlı (bkz. [Analiz 1](../analiz-1/diziler.html#def-sinirli-dizi)) olsun. O hâlde
...
::: {.ispat}
**Adım 1: $(a_n)$ yakınsaktır.** $(a_n)$ monoton ve sınırlı olduğundan monoton yakınsaklık teoremi (bkz. [Analiz 1](../analiz-1/monoton-diziler-ve-e-sayisi.html#thm-monoton-yakinsaklik)) gereği yakınsaktır. Limitine $a$ diyelim:
...
- *Sıfıra yakınsama.* Limit aritmetiği (bkz. [Analiz 1](../analiz-1/limit-islemleri-ve-sikistirma.html#thm-limit-aritmetigi)) gereği

$$\lim_{n \to \infty} \alpha_n = \lim_{n \to \infty} (a_n - a) = a - a = 0.$$

Öte yandan $\sum b_k$ yakınsak olduğundan kısmi toplamlar dizisi $(B_n)$ yakınsaktır; yakınsak her dizi sınırlıdır (bkz. [Analiz 1](../analiz-1/dizilerde-yakinsaklik.html#thm-yakinsak-sinirli)), yani $|B_n| \le M$ olacak biçimde bir $M > 0$ vardır.
...
olur. $\sum_{j=1}^{\infty} \dfrac{1}{\sqrt{j}} = \sum_{j=1}^{\infty} \dfrac{1}{j^{1/2}}$ serisi $p = \dfrac{1}{2} \le 1$ olduğundan ıraksaktır (@thm-p-serisi). Buradan $\sum_{j} \dfrac{1}{\sqrt{2}} \cdot \dfrac{1}{\sqrt{j}}$ serisinin de ıraksadığı çıkar; yakınsak olsaydı $\sqrt{2}$ ile çarpılmışı olan $\sum_{j} \dfrac{1}{\sqrt{j}}$ de yakınsak olurdu (@thm-seri-cebirsel-islemler). Küçük olan seri ıraksadığından, karşılaştırma testi (@thm-karsilastirma-testi) gereği $\sum_{j=1}^{\infty} \dfrac{1}{\sqrt{2j-1}}$ serisi de ıraksaktır. Bu seri negatif olmayan terimlidir; negatif olmayan terimli bir serinin ıraksaması, kısmi toplamlarının üstten sınırsız olması demektir (@thm-sinirli-kismi-toplam-olcutu) ve artan sınırsız bir dizi $+\infty$'a gider (bkz. [Analiz 1](../analiz-1/monoton-diziler-ve-e-sayisi.html#thm-monoton-iraksak)). Öyleyse
...
**Adım 5: Sonuç.** $(s_n)$ dizisinin bir alt dizisi olan $(s_{2n})$ sınırsız olduğundan $(s_n)$ de sınırsızdır; sınırsız bir dizi yakınsak olamaz (bkz. [Analiz 1](../analiz-1/dizilerde-yakinsaklik.html#thm-yakinsak-sinirli)). Demek ki $\sum (-1)^{k+1} a_k$ serisi ıraksaktır.
...
*Sıfıra gider:* $a_k = \dfrac{k}{k^2+1} = \dfrac{1/k}{1 + 1/k^2} \to \dfrac{0}{1} = 0$ (bkz. [Analiz 1](../analiz-1/limit-islemleri-ve-sikistirma.html#thm-limit-aritmetigi)).
...
**e)** Genel terime bakalım: $a_k = \left( 1 + \dfrac{1}{k} \right)^{k}$. Bu dizinin limiti $e$ sayısıdır (bkz. [Analiz 1](../analiz-1/monoton-diziler-ve-e-sayisi.html#thm-e-dizisi)):
...
olur (bkz. [Analiz 1](../analiz-1/limit-islemleri-ve-sikistirma.html#thm-limit-aritmetigi)). @cor-leibniz-testi gereği $\sum_{k \ge 2} (-1)^k c_k$ yakınsaktır.
