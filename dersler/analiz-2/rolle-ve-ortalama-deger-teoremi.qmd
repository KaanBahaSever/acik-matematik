...
::: {.ispat}
$f$ kapalı ve sınırlı $[a,b]$ aralığında sürekli olduğundan, Weierstrass ekstremum teoremi (bkz. [Analiz 1](../analiz-1/ara-deger-ve-weierstrass-teoremleri.html#thm-weierstrass-ekstremum)) gereği bu aralıkta bir en büyük ve bir en küçük değerini **alır**: öyle $x_M, x_m \in [a,b]$ noktaları vardır ki her $x \in [a,b]$ için
...
**c) Kapalı aralıkta süreklilik bozuluyor.** Bu fonksiyon için $f(0) = 0$ ve $f(1) = 0$, yani $f(0) = f(1)$'dir; ayrıca $(0,1)$ açık aralığında $f(x) = x$ olduğundan $f$ orada türevlenebilirdir ve $f'(x) = 1$'dir. Eksik olan tek şey **kapalı** aralıkta sürekliliktir: $x \to 1^-$ iken $f(x) = x \to 1$ olmasına karşın $f(1) = 0$'dır, dolayısıyla $f$, $1$ noktasında sürekli değildir (bkz. [Analiz 1](../analiz-1/sureklilik.html#def-sureklilik)). Sonuç yine bozulur: $(0,1)$ üzerinde $f'(x) = 1 \ne 0$'dır.
...
- $f$ bir polinomdur; polinomlar $\mathbb{R}$'nin tamamında süreklidir (bkz. [Analiz 1](../analiz-1/sureklilik.html#cor-polinom-surekli)), özel olarak $\left[ 0, \sqrt{3} \right]$ üzerinde süreklidir.
...
::: {.cozum}
$f(x) = x^3 + 3x + 1$ olsun. $f$ bir polinom olduğundan $\mathbb{R}$ üzerinde sürekli (bkz. [Analiz 1](../analiz-1/sureklilik.html#cor-polinom-surekli)) ve türevlenebilirdir.
...
$f$, $[-1,0]$ kapalı aralığında süreklidir ve uçlardaki değerleri zıt işaretlidir. Ara değer teoremi (bkz. [Analiz 1](../analiz-1/ara-deger-ve-weierstrass-teoremleri.html#thm-ara-deger)), daha doğrusu onun sıfır yeri biçimi (bkz. [Analiz 1](../analiz-1/ara-deger-ve-weierstrass-teoremleri.html#thm-bolzano-sifir)) gereği
...
$f$ sabitse her $c \in (a,b)$ noktası işi görür. $f$ sabit değilse $f(d) \ne L$ olan bir $d \in (a,b)$ vardır. Önce $f(d) > L$ olduğunu varsayalım ve $\varepsilon := f(d) - L > 0$ diyelim. $x \to a^+$ iken $f(x) \to L$ olduğundan ($a$ sonluysa [Sağ Limit ve Sol Limit](../analiz-1/tek-yonlu-limitler.html#def-tek-yonlu-limit), $a = -\infty$ ise [Sonsuzdaki Limit](../analiz-1/sonsuz-limitler-ve-asimptotlar.html#def-sonsuzda-limit) anlamında) öyle bir $\alpha \in (a,d)$ vardır ki
...
Şimdi $[\alpha, \beta] \subseteq (a,b)$ kapalı ve sınırlı aralığını ele alalım. $f$, $(a,b)$ üzerinde türevlenebilir olduğundan orada süreklidir (@thm-turev-sureklilik); Weierstrass ekstremum teoremi (bkz. [Analiz 1](../analiz-1/ara-deger-ve-weierstrass-teoremleri.html#thm-weierstrass-ekstremum)) gereği $f$ bu aralıkta **maksimum** değerini bir $c \in [\alpha,\beta]$ noktasında alır. $d \in (\alpha,\beta)$ olduğundan
...
$f$, $(a,b)$ üzerinde türevlenebilir olduğundan orada süreklidir (@thm-turev-sureklilik); özel olarak $[\alpha, \beta] \subseteq (a,b)$ kapalı ve sınırlı aralığında süreklidir. Weierstrass ekstremum teoremi (bkz. [Analiz 1](../analiz-1/ara-deger-ve-weierstrass-teoremleri.html#thm-weierstrass-ekstremum)) gereği $f$ bu aralıkta **minimum** değerini bir $c \in [\alpha, \beta]$ noktasında alır. $d \in (\alpha, \beta)$ olduğundan
...
- $\ell$ bir birinci dereceden polinomdur; $\mathbb{R}$'de süreklidir ve türevlenebilirdir. $f$ ise hipotez gereği $[a,b]$'de sürekli, $(a,b)$'de türevlenebilirdir. Sürekli fonksiyonların farkı sürekli (bkz. [Analiz 1](../analiz-1/sureklilik.html#thm-sureklilik-aritmetigi)), türevlenebilir fonksiyonların farkı türevlenebilir olduğundan (@thm-turev-aritmetigi), $g$ de $[a,b]$'de sürekli ve $(a,b)$'de türevlenebilirdir.
- Uç değerler:
...
::: {.ispat}
$x_1, x_2 \in I$ ve $x_1 < x_2$ olsun. $I$ bir aralık olduğundan $[x_1, x_2] \subseteq I$'dır (bkz. [Analiz 1](../analiz-1/siralama-aksiyomlari.html#def-aralik)). Bu durumda:
...
**1\. $h$ hipotezleri sağlar.** $f$ ve $g$, $I$ üzerinde sürekli olduğundan farkları da süreklidir (bkz. [Analiz 1](../analiz-1/sureklilik.html#thm-sureklilik-aritmetigi)). Aynı biçimde, $f$ ve $g$ her iç noktada türevlenebilir olduğundan farkları da türevlenebilirdir ve türev farkın türevidir (@thm-turev-aritmetigi):
...
3. Her $x \in (a,b)$ için $f'(x) \ge 0$ ise, $f$ fonksiyonu $[a,b]$ üzerinde **artandır** (bkz. [Analiz 1](../analiz-1/tek-yonlu-limitler.html#def-monoton-fonksiyon)), yani azalmayandır: $x_1 < x_2$ iken $f(x_1) \le f(x_2)$.
...
olur; $h < 0$ için de pay ve payda birlikte işaret değiştirdiğinden oran yine negatif değildir. Limit sıralamayı koruduğundan (bkz. [Analiz 1](../analiz-1/limitlerde-siralama-ve-sikistirma.html#thm-limit-siralama-fonksiyon)) $f'(x) \ge 0$ bulunur. Demek ki türevle monotonluk arasındaki tam ilişki şöyledir:
...
Ortalama değer teoreminin fark biçimi $f(y) - f(x) = f'(c)(y-x)$, türevin **büyüklüğü** hakkındaki bilgiyi doğrudan fonksiyon değerlerinin farkına aktarır. Türev sınırlıysa fonksiyon "fazla hızlı" değişemez; bu, Analiz 1'de tanımladığımız Lipschitz koşuludur (bkz. [Analiz 1](../analiz-1/duzgun-sureklilik-ve-cantor-teoremi.html#def-lipschitz-kosulu)).
...
- $f$ ve $g$, $[a,b]$ üzerinde sürekli olduğundan sabitlerle çarpımları ve farkları da süreklidir (bkz. [Analiz 1](../analiz-1/sureklilik.html#thm-sureklilik-aritmetigi)); yani $\varphi$, $[a,b]$ üzerinde süreklidir.
...
$f$ bir polinom olduğundan $[0,2]$ üzerinde sürekli (bkz. [Analiz 1](../analiz-1/sureklilik.html#cor-polinom-surekli)) ve $(0,2)$ üzerinde türevlenebilirdir. Uç değerler $f(0) = 0$ ve $f(2) = 2 \cdot 1 \cdot 0 = 0$ olduğundan $f(0) = f(2)$'dir; üç hipotez de sağlanır.
...
*Varlık.* $f(0) = -1 < 0$ ve $f(1) = 1 + 2 - 1 = 2 > 0$'dır. $f$, $[0,1]$ üzerinde sürekli ve uçlardaki değerleri zıt işaretli olduğundan [Bolzano Teoremi (Sıfır Yeri Teoremi)](../analiz-1/ara-deger-ve-weierstrass-teoremleri.html#thm-bolzano-sifir) gereği $f(k) = 0$ olan bir $k \in (0,1)$ vardır.
...
Üstel fonksiyon her yerde sürekli ve türevlenebilir olduğundan (@thm-ustel-turevi), $F$ de $[a,b]$ üzerinde sürekli ve $(a,b)$ üzerinde türevlenebilirdir (bkz. [Analiz 1](../analiz-1/sureklilik.html#thm-sureklilik-aritmetigi), @thm-turev-aritmetigi). Uç değerler:
