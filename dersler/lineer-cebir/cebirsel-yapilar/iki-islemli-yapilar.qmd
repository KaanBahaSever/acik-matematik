...
$$\begin{aligned}
(a+bi) \oplus (c+di) &= (a+c) + (b+d)i\\[1mm]
&= (c+a) + (d+b)i = (c+di) \oplus (a+bi)
\end{aligned}$$
...
$$\begin{aligned}
(a,5^{b}) \oplus \big[(c,5^{d}) \oplus (e,5^{f})\big] &= \big(a + (c+e),\ 5^{b+(d+f)}\big)\\[1mm]
&= \big[(a,5^{b}) \oplus (c,5^{d})\big] \oplus (e,5^{f})
\end{aligned}$$
...
$$\begin{aligned}
(a,5^{b}) \odot \big[(c,5^{d}) \oplus (e,5^{f})\big] &= (a,5^{b}) \odot \big(c+e,\ 5^{d+f}\big)\\[1mm]
