...
description: "Bir tabanda matris gösterimi almanın toplama, skalerle çarpma ve bileşke ile uyumu; operatörler cebri A(V) ile kare matrisler cebri arasındaki izomorfi."
---
...
dönüşümü $A(V)$'den $\mathcal{A}$ üzerine bir **vektör uzayı izomorfisidir**; yani bire bir ve örtendir, ayrıca her $S, T \in A(V)$ ve $k \in K$ için

$$[S + T]_e = [S]_e + [T]_e \qquad \text{ve} \qquad [kT]_e = k\,[T]_e$$

::: {.ispat}
**Dönüşüm bire birdir.** Farklı operatörlerin matrisleri farklıdır: $S \neq T$ ise $V$'nin bir taban vektöründe $S(e_i) \neq T(e_i)$ olmalıdır (aksi hâlde [tabanda belirlenme](../lineer-tasvirler/koordinatlar-ve-izomorfizm.qmd) gereği $S = T$ olurdu), dolayısıyla matrislerin $i$. sütunları farklıdır ve $[S]_e \neq [T]_e$'dir.
...
Üç teoremi bir araya getirince şu sonuç çıkar: **$A(V)$ ile $n$. mertebeden kare matrisler cebri, bir taban seçimiyle aynı yapıdır.** Operatörlerle yapılan bütün hesaplar matrislerle, matrislerle yapılan bütün hesaplar operatörlerle yürütülebilir.

::: {.callout-note appearance="simple"}
Bu izomorfi, [operatör cebrinde](../operatorler/bileske-ve-cebir.qmd) karşılaştığımız tuhaflıkların matrislerdekiyle aynı olmasını açıklar: matris çarpımının komutatif olmaması ve sıfır bölenlerinin bulunması ile operatör çarpımınınkiler aynı olgunun iki yüzüdür. Boyut sayımı da tutar: $\dim A(V) = n^{2}$ ve $n$. mertebeden kare matrislerin uzayının boyutu da $n^{2}$'dir.
:::
...
::: {#exr-matris-cebri name="İzomorfinin Kullanımı"}
...
**Yeterlik.** $[T]_e$ regüler olsun; $[T]_e \cdot M = M \cdot [T]_e = I_n$ olacak bir $M$ kare matrisi vardır. İzomorfi örten olduğundan $[F]_e = M$ olacak bir $F \in A(V)$ bulunur. O hâlde

$$[TF]_e = [T]_e \cdot [F]_e = I_n = [I]_e, \qquad [FT]_e = [F]_e \cdot [T]_e = I_n = [I]_e$$

İzomorfi bire bir olduğundan $TF = I$ ve $FT = I$'dır; yani $F = T^{-1}$ ve $T$ tersinirdir.
