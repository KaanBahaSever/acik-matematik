...
::: {.ispat}
**Adım 1: En küçük ve en büyük değerler alınır.** $[a,b]$ kapalı ve sınırlı bir aralık, $f$ ise bu aralıkta süreklidir. Weierstrass ekstremum teoremi (@thm-weierstrass-ekstremum) gereği $f$, $[a,b]$ üzerinde en küçük ve en büyük değerlerini gerçekten alır: öyle $p, q \in [a,b]$ noktaları vardır ki
...
yani $G$, $K$ sabitiyle Lipschitz koşulunu sağlar (@def-lipschitz-kosulu). Lipschitz koşulunu sağlayan her fonksiyon düzgün süreklidir (@thm-lipschitz-duzgun), düzgün süreklilik ise sürekliliği gerektirir (@prp-duzgun-surekli-ise-surekli); demek ki $G$, $[a,b]$ üzerinde süreklidir. $[a,b]$ kapalı ve sınırlı olduğundan $G$ burada en küçük ve en büyük değerlerini alır (@thm-weierstrass-ekstremum):
...
buluruz. $G$ sürekli olduğundan $m_G$ ve $M_G$ değerlerini $[a,b]$'de gerçekten alır; ara değer teoremi (@thm-ara-deger) gereği bu iki değer arasındaki her sayıyı da alır. Öyleyse
...
**(ii) İlk yön: türev OMT'si $\Rightarrow$ integral OMT'si.** $F$, $(a,b)$ üzerinde türevlenebilir olduğundan orada süreklidir (@thm-turev-sureklilik). Uç noktalar dâhil bütün $[a,b]$ üzerinde de süreklidir: $f$ sürekli olduğundan sınırlıdır (@thm-surekli-sinirli), yani her $t \in [a,b]$ için $|f(t)| \le K$ olacak biçimde bir $K$ sabiti vardır; integral için üçgen eşitsizliği (@thm-integral-ucgen-esitsizligi) gereği $a \le x < y \le b$ için $|F(y) - F(x)| = \left| \int_x^{y} f(t)\,dt \right| \le K(y-x)$ olur. Demek ki $F$ Lipschitz koşulunu sağlar (@def-lipschitz-kosulu) ve dolayısıyla süreklidir (@thm-lipschitz-duzgun, @prp-duzgun-surekli-ise-surekli). Özetle $F$, $[a,b]$ üzerinde sürekli ve $(a,b)$ üzerinde türevlenebilirdir. @thm-ortalama-deger gereği
...
::: {.cozum}
Öncelikle ifadenin anlamlı olması için $\frac{1}{n} \le \delta$, yani $n \ge \frac{1}{\delta}$ olmalıdır. Arşimet özelliği gereği (@thm-arsimet) böyle bir $n$ vardır ve limit ancak yeterince büyük $n$'ler için yazılan terimlere bağlı olduğundan bunun bir sakıncası yoktur. Bundan sonra $n \ge \frac{1}{\delta}$ varsayılacaktır.
...
ve $\frac{1}{n} \to 0$ olduğundan sıkıştırma teoremi (@thm-sikistirma-dizi) gereği $x_n \to 0$'dır. $f$, $0$ noktasında sürekli olduğundan süreklilik için dizisel ölçüt (@thm-sureklilik-dizisel-olcut) gereği
