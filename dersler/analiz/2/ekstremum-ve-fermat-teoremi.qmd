...
Tanımlarda $x_0$ noktasının $\varepsilon$-komşuluğu (@def-komsuluk) gösterimini kullanacağız:
...
Fark oranı (@def-fark-orani) $x_0$'ın sağındaki bütün noktalarda $\le 0$ olduğundan, sağdan limit alındığında bu eşitsizlik korunur (@thm-limit-siralama-fonksiyon; sabit $0$ fonksiyonuyla karşılaştırıyoruz). Sağdan limit tam olarak sağdan türevdir (@def-tek-yonlu-turev):
...
$x \to a^+$ limitine geçersek, sıralama limitte korunacağından (@thm-limit-siralama-fonksiyon)
...
::: {.cozum}
**Mutlak minimum.** Mutlak değerin tanımı gereği (@def-mutlak-deger) her $x \in \mathbb{R}$ için $|x| \ge 0$'dır ve $f(0) = |0| = 0$'dır. Demek ki her $x$ için
...
Weierstrass ekstremum teoremi (@thm-weierstrass-ekstremum), kapalı ve sınırlı bir $[a,b]$ aralığında sürekli olan bir $f$ fonksiyonunun bu aralıkta hem mutlak maksimumunu hem mutlak minimumunu **aldığını** söyler; yani öyle $x_{\min}, x_{\max} \in [a,b]$ noktaları vardır ki her $x \in [a,b]$ için
...
1. **Varlık.** @thm-weierstrass-ekstremum gereği $f$ mutlak maksimumunu ve mutlak minimumunu $[a,b]$ üzerinde alır. Bu adım atlanamaz: aradığımız şeyin var olduğunu bilmeden aday karşılaştırmak anlamsızdır.
...
::: {.cozum}
**1\. adım: varlık.** $f$ bir polinomdur, dolayısıyla $\mathbb{R}$'de ve özel olarak $[-2,3]$ üzerinde süreklidir (@cor-polinom-surekli). $[-2,3]$ kapalı ve sınırlı bir aralık olduğundan @thm-weierstrass-ekstremum gereği $f$ bu aralıkta mutlak maksimumunu ve mutlak minimumunu alır. Öyleyse aramamız boşa çıkmayacaktır.
...
**Yordam.** $A$ bir polinomdur; $[0,20]$ üzerinde süreklidir (@cor-polinom-surekli) ve @thm-weierstrass-ekstremum gereği mutlak maksimumunu alır. Her yerde türevlenebilir olduğundan ikinci türden kritik nokta yoktur:
...
::: {.cozum}
**a)** $f$ bir polinomdur; $[-2,2]$ üzerinde süreklidir (@cor-polinom-surekli) ve @thm-weierstrass-ekstremum gereği mutlak ekstremumlarını alır. Her yerde türevlenebilirdir, dolayısıyla adaylar yalnızca durağan noktalar ve uçlardır:
...
Uçlarda $V(0) = 0$ ve $V(6) = 0$'dır; bunlar yozlaşmış hâllerdir ama fonksiyonu kapalı aralığa genişletmek varlık için işimize yarar. $V$ bir polinomdur, $[0,6]$ üzerinde süreklidir ve @thm-weierstrass-ekstremum gereği mutlak maksimumunu alır. Türevini çarpım kuralıyla (@thm-turev-aritmetigi) ve zincir kuralıyla (@thm-zincir-kurali) hesaplayalım:
