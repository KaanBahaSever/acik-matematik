...
*Doğrulama.* $\bigl(\ln \lvert \sec x + \tan x\rvert\bigr)' = \dfrac{\sec x\tan x + \sec^{2}x}{\sec x + \tan x} = \dfrac{\sec x\left(\tan x + \sec x\right)}{\sec x + \tan x} = \sec x$. Bu sonucu birazdan $\int \sec^{3}x\,dx$ hesabında kullanacağız.
