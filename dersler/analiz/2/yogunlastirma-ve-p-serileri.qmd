...
$\sum_{k=0}^{\infty} 2^k a_{2^k}$ yakınsak olsun ve toplamı $T$ ile gösterelim. $(t_k)$ artan olduğundan her $k$ için $t_k \le T$'dir. Şimdi herhangi bir $n \in \mathbb{N}$ alalım. Bernoulli eşitsizliği (@thm-bernoulli-esitsizligi) gereği her $k \ge 0$ için
...
Demek ki $(s_n)$ artan ve üstten $T$ ile sınırlıdır; @thm-monoton-yakinsaklik gereği yakınsaktır. Yani $\sum_{k=1}^{\infty} a_k$ yakınsaktır ve toplamı $T$'yi aşmaz.
...
Demek ki $(t_k)$ artan ve üstten $2S$ ile sınırlıdır; @thm-monoton-yakinsaklik gereği yakınsaktır.
...
biçimindedir. $0 < r < 1$ ise $r^{m+1} \to 0$ olduğundan (@thm-geometrik-dizi) kısmi toplamlar $\dfrac{1}{1-r}$ sayısına yakınsar, yani seri yakınsaktır. $r \ge 1$ ise her $k$ için $r^k \ge 1$ olur, genel terim sıfıra gitmez ve seri ıraksar.
...
olur (@thm-limit-aritmetigi). Dolayısıyla
...
olur (@thm-integral-siralama). Bu $F$, tam olarak @def-integral-fonksiyonu'ndaki integral fonksiyonudur. Artan bir $F$ üstten sınırlı ise, $M = \sup_{b > a} F(b)$ diyelim. Supremumun karakterizasyonu (@thm-supremum-epsilon) gereği her $\varepsilon > 0$ için $F(b_0) > M - \varepsilon$ olan bir $b_0$ vardır; $F$ artan olduğundan $b \ge b_0$ olan her $b$ için $M - \varepsilon < F(b) \le M$ olur. Demek ki $\lim_{b \to \infty} F(b) = M$'dir. $F$ üstten sınırlı değilse $F(b) \to +\infty$ olur. Kısacası:
...
çıkar. $f > 0$ olduğundan $(s_n)$ artandır; üstten sınırlı olduğunu da gösterdik. @thm-monoton-yakinsaklik gereği $(s_n)$ yakınsaktır, yani $\sum_{n=1}^{\infty} f(n)$ yakınsaktır.
...
Şimdi herhangi bir $b > 1$ alalım. $b \le n$ olacak biçimde bir $n$ doğal sayısı seçelim (Arşimet özelliği, @thm-arsimet). $f > 0$ olduğundan $\int_1^b f \le \int_1^n f = I_n \le S$ olur. Demek ki $\left\{ \int_1^b f : b > 1 \right\}$ kümesi üstten $S$ ile sınırlıdır; yukarıdaki gözlem gereği $\int_1^{\infty} f$ yakınsaktır ve değeri $S$'yi aşmaz.
