...
Önce ifadeyi kuralım. $f$'nin görüntü kümesi $g$'nin tanım kümesinin içinde kalmalı ki bileşke tanımlı olsun (@def-bileske). Türev yerel bir kavram olduğu için $x_0$'ın $I$'nın **iç** noktası, $y_0 = f(x_0)$'ın da $J$'nin iç noktası olmasını isteyeceğiz; en rahat yol $I$ ve $J$'yi açık aralık almaktır.
...
bulunur. Artık iki çarpanın da limiti vardır; limitin çarpımla uyuşmasından (@thm-limit-aritmetigi-fonksiyon)
...
kümesinin bir yığılma noktası $x_0$'dır (@def-yigilma-noktasi).
...
olup sabit sıfırdır. $f'(x_0)$ var olduğundan, limitin dizisel ölçütü (@thm-dizisel-olcut) gereği bu dizideki değerlerin limiti de $f'(x_0)$ olmalıdır; sabit sıfır dizisinin limiti sıfır olduğundan
...
olur. Ters üçgen eşitsizliğinden (@cor-ters-ucgen-esitsizligi) bu $y$'ler için

$$\left| \frac{g(y) - g(y_0)}{y - y_0} \right| < |g'(y_0)| + 1 =: M$$

bulunur. Yani $y$, $y_0$'a yeterince yakın ve $y \neq y_0$ olduğu sürece, $g$'nin fark oranı $M$ sabitiyle sınırlıdır. Sınırlılığın kaynağı budur: **türevlenebilir bir fonksiyonun fark oranının limiti vardır, limiti olan bir fonksiyon ise o noktanın yakınında sınırlıdır** (@thm-limit-yerel-sinirlilik).
...
eşitsizliği geçerlidir. Sağ taraf $x \to x_0$ iken $M \cdot |f'(x_0)| = M \cdot 0 = 0$'a gider (mutlak değerin limitle uyuşması için @thm-mutlak-deger-limiti-fonksiyon). Sıkıştırma teoremi (@thm-sikistirma-fonksiyon) gereği
...
yazmak yeter; süreklilik ile limitin denkliği (@prp-sureklilik-limit-denklik) gereği $\varphi$, $x_0$'da süreklidir.
...
Geriye $\Phi$'nin $x_0$'da sürekli olduğunu görmek kaldı. $\varphi$ zaten $x_0$'da süreklidir. $f$, $x_0$'da süreklidir (türevlenebilirlikten) ve $\psi$, $f(x_0) = y_0$'da süreklidir; bileşkenin sürekliliği (@thm-bileske-surekliligi) gereği $\psi \circ f$ de $x_0$'da süreklidir. Sürekli iki fonksiyonun çarpımı sürekli olduğundan (@thm-sureklilik-aritmetigi) $\Phi = (\psi \circ f)\cdot \varphi$ fonksiyonu $x_0$'da süreklidir.
...
**(b)** $n$ üzerinden tümevarım yapalım (@thm-tumevarim-ilkesi).
...
ve $|x| \to 0$'dır. Sıkıştırma teoremi (@thm-sikistirma-fonksiyon) gereği fark oranının limiti $0$'dır; yani $f$, $0$'da türevlenebilirdir ve

$$f'(0) = 0.$$

(Aynı sonucu, sınırlı bir fonksiyonla sıfıra giden bir fonksiyonun çarpımının sıfıra gitmesinden de okuyabiliriz: @thm-sinirli-carpi-sifir-fonksiyon.)
...
**Neden durum ayrımı gerekli?** Herhangi bir $\delta > 0$ alalım. Arşimet özelliği (@thm-arsimet) gereği $n\pi > \frac{1}{\delta}$ olan bir $n \in \mathbb{N}$ vardır; bu $n$ için $0 < x_n < \delta$'dır. Demek ki $0$'ın **her** delinmiş komşuluğunda $f(x) = f(0)$ olan noktalar bulunur. Sonuç olarak
...
demektir. Limit sıfırdan farklı olduğundan, limitin bölmeyle uyuşması (@thm-limit-aritmetigi-fonksiyon) gereği
...
$\varphi(x_0) = f'(x_0) \neq 0$ olduğundan ve $\varphi$, $x_0$'da sürekli olduğundan, sürekliliğin işaret koruma özelliği (@lem-surekli-isaret-koruma) gereği $x_0$'ın bir $U = (x_0 - \rho, x_0 + \rho) \cap I$ komşuluğunda $\varphi(x) \neq 0$'dır. $f^{-1}$ süreklidir, dolayısıyla $y_0$'ın $J$ içinde öyle bir $V$ komşuluğu vardır ki $f^{-1}(V) \subseteq U$ olur; $V$'yi $J$'nin bir alt aralığı seçebiliriz.
...
Sağdaki katsayı fonksiyonu $y \mapsto \dfrac{1}{\varphi(f^{-1}(y))}$, $y_0$'da süreklidir: $f^{-1}$, $y_0$'da; $\varphi$, $x_0$'da süreklidir (bileşke için @thm-bileske-surekliligi) ve payda $y_0$'da sıfırdan farklıdır (@thm-sureklilik-aritmetigi). Türevlenebilirlik yerel bir özellik olduğundan @lem-caratheodory'yi $V$ aralığı üzerinde $f^{-1}$'e uygulayabiliriz: $f^{-1}$, $y_0$'da türevlenebilirdir ve
...
::: {.cozum}
**Karekök.** $f(x) = x^2$ fonksiyonu $(0,\infty)$ üzerinde kesin artandır ($0 < a < b$ ise $a^2 < b^2$), süreklidir ve $(0,\infty)$ üzerine örtendir (her $y > 0$ için $\sqrt{y}$ vardır: @thm-n-inci-kok-varlik). Dolayısıyla birebir ve örtendir; tersi $f^{-1}(y) = \sqrt{y}$'dir.
...
**Örtenlik.** $f$ bir polinom olduğundan süreklidir (@cor-polinom-surekli). $y \in \mathbb{R}$ verilsin ve $t = |y| + 1 \ge 1$ diyelim. O zaman

$$f(t) = t^3 + t > t = |y| + 1 > y, \qquad f(-t) = -t^3 - t < -t = -|y| - 1 < y.$$

$f$, $[-t, t]$ üzerinde sürekli ve $f(-t) < y < f(t)$ olduğundan ara değer teoremi (@thm-ara-deger) gereği $f(c) = y$ olan bir $c \in (-t, t)$ vardır. Demek ki $f$ örtendir.
...
Payda her zaman pozitif olduğundan $u'(x) > 0$'dır. (Aynı sonucu $u(x) = x\,(1+x^2)^{-1/2}$ yazıp çarpım kuralı ile zincir kuralını birlikte kullanarak da bulabiliriz: $u'(x) = (1+x^2)^{-1/2} + x \cdot \left( -\frac{1}{2} \right)(1+x^2)^{-3/2}\cdot 2x = (1+x^2)^{-3/2}\big[ (1+x^2) - x^2 \big] = (1+x^2)^{-3/2}$.)
...
*Örtenlik.* $f$ süreklidir (@cor-polinom-surekli). $y \in \mathbb{R}$ verilsin ve $t = |y| + 1 \ge 1$ alalım. $t \ge 1$ olduğundan $t^5 \ge 1$ ve

$$f(t) = t^5 + 3t + 1 \ge 3t + 2 > t = |y| + 1 > y,$$

$$f(-t) = -t^5 - 3t + 1 \le -3t \le -t = -|y| - 1 < y.$$

$f$, $[-t, t]$ aralığında sürekli ve $f(-t) < y < f(t)$ olduğundan ara değer teoremi (@thm-ara-deger) gereği $f(c) = y$ olan bir $c \in (-t, t)$ vardır. Demek ki $f$ örtendir.
