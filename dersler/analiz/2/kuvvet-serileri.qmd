...
Yakınsak her dizi sınırlıdır (@thm-yakinsak-sinirli). Dolayısıyla $\big( a_n (x_1 - x_0)^n \big)_{n \ge 0}$ dizisi sınırlıdır: öyle bir $M > 0$ sayısı vardır ki her $n \in \mathbb{N}$ için
...
::: {.ispat}
**Adım 1: $S$ boş değildir.** Seri merkezinde her zaman yakınsadığından ($x = x_0$ için toplam $a_0$'dır) $0 \in S$'dir. Ayrıca $S \subseteq [0, \infty)$ olduğundan $S$ alttan sınırlıdır. Öyleyse $S$ üstten sınırlıysa, supremumun varlık aksiyomu gereği $\sup S$ vardır (@def-supremum-infimum) ve $R = \sup S \ge 0$'dır.

**Adım 2: $|x - x_0| < R$ ise seri $x$'te mutlak yakınsar.** $x$, $|x-x_0| < R$ koşulunu sağlasın.

*Durum (a): $R$ sonlu.* $|x - x_0| < R = \sup S$ olduğundan, $|x-x_0|$ sayısı $S$ için bir üst sınır olamaz; supremumun karakterizasyonu (@thm-supremum-epsilon) gereği öyle bir $s \in S$ vardır ki
...
Bu önerme, girişte söylediğimiz "yalnızca üç seçenek vardır" iddiasını da ispatlamış olur: $R = 0$ ise $I = \{x_0\}$, $R = +\infty$ ise $I = \mathbb{R}$, $0 < R < \infty$ ise $I$, uçları $x_0 \pm R$ olan bir aralıktır. Her üç durumda da $I$ bir aralıktır (@def-aralik).
...
olsun (@def-ust-alt-limit). Burada $r \in [0, +\infty]$'dur. Serinin yakınsaklık yarıçapı
...
Bu dizinin limiti $e$ sayısıdır (@thm-e-dizisi). @thm-oran-ile-yaricap gereği
