...
$f$, $[a,b]$ üzerinde sürekli olsun. Her $[x_{k-1}, x_k]$ kapalı ve sınırlı bir aralık olduğundan, Weierstrass ekstremum teoremi (@thm-weierstrass-ekstremum) gereği $f$ bu aralıkta en büyük ve en küçük değerini **gerçekten alır**: öyle $u_k, v_k \in [x_{k-1}, x_k]$ noktaları vardır ki
...
::: {.ispat}
**(i\) Sıkıştırma eşitsizliği.** $T = \{t_1, \dots, t_n\}$ verilsin. Her $k$ için $t_k \in [x_{k-1}, x_k]$ olduğundan, infimum ile supremumun tanımı (@def-supremum-infimum) gereği
...
**Alt toplam bir infimumdur.** Aynı akıl yürütmeyi infimum için tekrarlayalım. (i\)'den $L(P,f)$, $\mathcal{S}$'nin bir alt sınırıdır. $\varepsilon > 0$ verilsin. Her $k$ için $m_k = \inf\{ f(x) : x \in [x_{k-1}, x_k] \}$ olduğundan, infimumun $\varepsilon$-nitelemesi (@thm-infimum-epsilon) gereği
...
**Adım 1: Üst toplamlar için uygun bir sabit bölünüş seçimi.** $\varepsilon > 0$ verilsin. $\overline{I}(f)$, $\{U(P,f) : P \text{ bölünüş}\}$ kümesinin infimumu olduğundan, infimumun $\varepsilon$-nitelemesi (@thm-infimum-epsilon) gereği öyle bir $P_0$ bölünüşü vardır ki
...
**Adım 5: Alt toplamlar.** Şimdi aynı yolu alt toplamlar için yürüyelim. $\varepsilon > 0$ verilsin. $\underline{I}(f)$, $\{L(P,f) : P \text{ bölünüş}\}$ kümesinin supremumu olduğundan, supremumun $\varepsilon$-nitelemesi (@thm-supremum-epsilon) gereği öyle bir $P_1$ bölünüşü vardır ki
...
Böyle bir $P$'nin var olduğuna dikkat edelim: Arşimet özelliği (@thm-arsimet) gereği $n > \dfrac{b-a}{\delta}$ olan bir $n \in \mathbb{N}$ vardır ve $[a,b]$'nin $n$ eşit parçaya bölünmesiyle elde edilen düzgün bölünüşün normu $\dfrac{b-a}{n} < \delta$'dır.
...
olur. Üçgen eşitsizliği (@thm-ucgen-esitsizligi) gereği bundan
...
**Adım 2: Böyle bir bölünüş seçmek.** Arşimet özelliği (@thm-arsimet) gereği $n > \dfrac{b-a}{\delta}$ olan bir $n \in \mathbb{N}$ vardır. $[a,b]$'yi $n$ eşit parçaya bölen düzgün bölünüşü alalım; normu $\dfrac{b-a}{n} < \delta$'dır. Bu bölünüşü $P = \{x_0, x_1, \dots, x_n\}$ ile gösterelim ve **sabitleyelim.**
...
Ters üçgen eşitsizliği (@cor-ters-ucgen-esitsizligi) gereği
...
Rasyonel sayıların yoğunluğu (@thm-rasyonellerin-yogunlugu) gereği $(x_{k-1}, x_k)$ aralığında bir $r$ rasyoneli vardır; $f(r) = 1$'dir. İrrasyonel sayıların yoğunluğu (@thm-irrasyonellerin-yogunlugu) gereği aynı aralıkta bir $s$ irrasyoneli vardır; $f(s) = 0$'dır. $f$'nin bütün değerleri $\{0,1\}$ kümesinde olduğundan
...
::: {.cozum}
**Kısa yol.** $f$, $[0,1]$ üzerinde sınırsızdır: $n \in \mathbb{N}$ için $x = \dfrac{1}{n} \in (0,1]$ noktasında $f\!\left( \dfrac{1}{n} \right) = n$ olur; Arşimet özelliği (@thm-arsimet) gereği $\{n : n \in \mathbb{N}\}$ kümesi üstten sınırsız olduğundan $f$'nin değerler kümesi de üstten sınırsızdır. @thm-integrallenebilir-sinirli gereği $f$ integrallenemez.
...
olacak biçimde bir $t_1 \in (0, x_1]$ vardır. Bu seçimle $\Delta x_1 = x_1 - 0 = x_1$ olduğundan $f(t_1)\Delta x_1 = \dfrac{x_1}{t_1} > C + A$ olur ve ters üçgen eşitsizliği (@cor-ters-ucgen-esitsizligi) gereği
...
**Adım 2: Normun sıfıra gitmesi.** $b - a > 0$ sabit olduğundan $\|P_n\| = \dfrac{b-a}{n} \to 0$'dır ($n \to \infty$). Bunu $\varepsilon$–$N$ diliyle de yazabiliriz: verilen $\delta > 0$ için, Arşimet özelliği (@thm-arsimet) gereği $N > \dfrac{b-a}{\delta}$ olan bir $N \in \mathbb{N}$ vardır ve $n \ge N$ olduğunda
...
Son iki terim $n \to \infty$ iken sıfıra gider (@thm-limit-aritmetigi), yani limit $\dfrac{1}{3}$'tür. İki hesap uyuşuyor.
...
**İntegrallenebilirlik.** $[0,1]$ üzerinde $1 + x \ge 1 > 0$ olduğundan $f$ bu aralıkta tanımlıdır ve sürekli fonksiyonların bölümü olarak süreklidir (@thm-sureklilik-aritmetigi). Süreklilik integrallenebilirliği verir.
...
**İntegrallenebilirlik.** $f(x) = \sqrt{x}$ fonksiyonu $[0,1]$ üzerinde süreklidir (@prp-mutlak-deger-karekok-surekli) ve $0 \le \sqrt{x} \le 1$ olduğundan sınırlıdır; öyleyse integrallenebilirdir.
...
seçimine karşılık gelir. $\sin$ süreklidir (@prp-sin-cos-surekli), $x \mapsto \pi x$ süreklidir ve bileşkeleri de süreklidir (@thm-bileske-surekliligi); öyleyse $f$ integrallenebilirdir. $F(x) = -\dfrac{\cos(\pi x)}{\pi}$ bir ilkeldir ve
...
$\cos u_n \to 1$ ve $\dfrac{\sin u_n}{u_n} \to 1$ (@thm-sinx-bolu-x) olduğundan limit yine $\dfrac{2}{\pi}$'dir.
...
**İntegrallenebilirlik.** Her $x$ için $1 + x^2 \ge 1 > 0$ olduğundan $f$, $\mathbb{R}$ üzerinde tanımlı ve süreklidir (@thm-sureklilik-aritmetigi); özellikle $[0,1]$ üzerinde integrallenebilirdir.
...
(d\) şıkkına göre $\ln a_n \to 2\ln 2 - 1$'dir. $\exp$ fonksiyonu sürekli olduğundan, süreklilik için dizisel ölçüt (@thm-sureklilik-dizisel-olcut) gereği

$$a_n = e^{\ln a_n} \longrightarrow e^{2\ln 2 - 1} = \frac{e^{\ln 4}}{e} = \frac{4}{e}.$$

Yani aranan limit $\dfrac{4}{e} \approx 1{,}4715$'tir.

**f)** Önce $g$'nin integrallenemediğini gösterelim. $g$ sınırlıdır: her $x$ için $|g(x)| = 1$'dir. $P = \{x_0, \dots, x_n\}$ herhangi bir bölünüş olsun. Her $[x_{k-1}, x_k]$ alt aralığı hem rasyonel hem irrasyonel sayı içerir (@thm-rasyonellerin-yogunlugu, @thm-irrasyonellerin-yogunlugu); $g$'nin aldığı değerler yalnızca $1$ ve $-1$ olduğundan
