...
önermesine, yani $x \in (A \cap B) \cup (A \cap C)$'ye denktir. Her adım bir denklik olduğundan iki küme aynı elemanlara sahiptir. İkinci eşitlik, $p \vee (q \wedge r) \equiv (p \vee q) \wedge (p \vee r)$ yasasıyla tıpatıp aynı biçimde çıkar: $x \in A \cup (B \cap C) \Leftrightarrow x \in A \vee (x \in B \wedge x \in C) \Leftrightarrow (x \in A \vee x \in B) \wedge (x \in A \vee x \in C) \Leftrightarrow x \in (A \cup B) \cap (A \cup C)$.
