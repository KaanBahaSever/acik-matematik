...
Kerckhoffs Prensibi'ni hatırlayalım: Düşman şifreleme algoritmasını ($x + k \pmod{26}$ yapıldığını) biliyor kabul edilir. Sistemin güvenliği sadece $k$ anahtarının gizliliğine dayanmalıdır. Ancak olası sadece 25 anahtar olduğu için, düşman şifreli bir metni ele geçirdiğinde hiçbir zekice matematiğe başvurmadan **"Kaba Kuvvet Saldırısı" (Brute Force Attack)** yaparak tüm ihtimalleri deneyebilir. Bir insan bile kağıt kalemle birkaç dakikada tüm 25 kaydırmayı test edip anlamlı kelimeyi bulabilir. 
...
::: details 🟢 Örnek 1: Açık metni "MATH" olan bir mesajı, k = 7 anahtarını kullanarak Sezar şifrelemesi ile şifreleyiniz.
**Çözüm:**
...
::: details 🔵 Örnek 2: k = 5 anahtarı ile şifrelenmiş olan "GTD" kapalı metnini deşifre ediniz.
**Çözüm:**
...
3. **D** $\to 3 \implies 3 - 5 = -2$ 
   *Eksi bir sonuç bulduğumuzda, pozitif denklik bulana kadar modül ekleriz:*
   $-2 \equiv -2 + 26 \equiv 24 \pmod{26} \implies \mathbf{Y}$

**Sonuç:** Şifreli `GTD` metninin açık hali `BOY` kelimesidir.
:::

::: details 🔴 Örnek 3: Düşman haberleşme hattından "WKLV" şifreli metnini ele geçirdiniz. Bunun aslında İngilizce "THIS" kelimesi olduğu bilindiğine göre, kullanılan Sezar anahtarını (k) cebirsel olarak bulunuz.
**Çözüm:**
