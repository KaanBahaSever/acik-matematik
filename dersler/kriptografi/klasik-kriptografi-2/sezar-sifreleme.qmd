...
```{=html}
<div class="calculator" id="caesar-calculator">
  ...
  <label>
    <span>Anahtar (k = <b data-role="k-label">3</b>)</span>
    <input type="range" data-role="k" min="0" max="25" value="3">
  </label>
...
<script>
(function () {
  const box = document.getElementById("caesar-calculator");
  const get = (role) => box.querySelector('[data-role="' + role + '"]');
  const mode = get("mode"), k = get("k"), kLabel = get("k-label");
  const input = get("input"), output = get("output");

  function compute() {
    const shift = Number(k.value);
    const direction = mode.value === "encrypt" ? shift : -shift;
    kLabel.textContent = shift;

    output.textContent = input.value.toUpperCase().replace(/[A-Z]/g, (letter) => {
      const x = letter.charCodeAt(0) - 65;
      return String.fromCharCode(((x + direction) % 26 + 26) % 26 + 65);
    });
  }

  [mode, k, input].forEach((el) => {
