...
* **Anahtar Uzayı ($\mathcal{K}$):** $k = (a, b)$ formunda olmak üzere; $a, b \in \mathbb{Z}_{26}$ ve $\operatorname{gcd}(a, 26) = 1$ koşulunu sağlayan tüm sıralı ikililerin kümesi.
...
Şifreleme fonksiyonunun çözülebilmesi için **mutlaka birebir (1-1) olması** gerekir. Eğer $a$ sayısı 26 ile aralarında asal değilse (yani $\operatorname{gcd}(a, 26) \neq 1$), fonksiyon birebirliğini kaybeder.

**Örnek bir felaket senaryosu:**
Diyelim ki kuralı ihlal edip $\operatorname{gcd}(2, 26) \neq 1$ olmasına rağmen $k = (2, 3)$ seçtik.
...
::: tip Toplam Anahtar Uzayı ve Euler'in Phi (Totient) Fonksiyonu
...
::: details 🟢 Örnek 1: Açık metni "MATH" olan bir mesajı, k = (5, 8) anahtarını kullanarak Afin şifrelemesi ile şifreleyiniz.
**Çözüm:**
...
::: details 🔵 Örnek 2: k = (7, 2) anahtarı ile şifrelenmiş olan "QCF" kapalı metnini deşifre ediniz.
**Çözüm:**
...
::: details 🔴 Örnek 3: Düşmandan ele geçirilen "AGIY" şifreli metninin, orijinalinde "OKAY" kelimesi olduğu bilinmektedir. Kullanılan (a, b) Afin anahtarını bulunuz.
**Çözüm:**
$e_k(x) \equiv ax + b \pmod{26}$ olduğunu biliyoruz. İlk iki harf üzerinden iki bilinmeyenli bir denklem sistemi kurarak sistemi çözebiliriz.
...
**Kritik Aşama:** Mod 26'da $4a \equiv 20$ denklemini çözerken direkt 4'e bölemeyiz. $\operatorname{gcd}(4, 26) = 2$ olduğu için bu denklemin iki farklı kökü vardır:
1. $a \equiv 5 \pmod{26}$
2. $a \equiv 5 + 13 = 18 \pmod{26}$

Ancak Afin kuralları gereği $\operatorname{gcd}(a, 26) = 1$ olmak zorundadır. $\operatorname{gcd}(18, 26) \neq 1$ olduğu için $a = 18$ değeri **geçersizdir.** Demek ki **$a = 5$** olmalıdır.
